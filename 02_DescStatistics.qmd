...
econ = read_rds("02_GenData/03_dem/econ.rds")
dem = read_rds("02_GenData/03_dem/dem.rds")
asthma = read_rds("02_GenData/03_dem/AsthmaHosp.rds")
# Load the location of all schools
location = read_rds("02_GenData/01_schools/location.rds")
# Load the data set on absenteeism
absent = read_rds("02_GenData/RawAbsences.rds")
...
BoroShp = st_read("01_RawData/04_shp/02_bgh", quiet = T) %>% 
  st_transform(., crs = 4326) %>% st_simplify(., preserveTopology = T)
# Load the shapefile on city neighborhoods
NtaShp = st_read("01_RawData/04_shp/03_ngh", quiet = T) %>% 
  st_transform(., crs = 4326) %>% st_simplify(., preserveTopology = T)
...
```{r}
aqif = read_rds("02_GenData/02_aqi/AqiForecast.rds")
aqi = read_rds("02_GenData/02_aqi/RealAqi.rds")
```
